-- =============================================================================
-- HiraLearn — Canonical content seed (11 courses, modules, lessons, tasks)
-- =============================================================================
-- HOW TO RUN:
...
--   * Sets each course's order_index so the UI shows them in canonical order
--     (HTML → CSS → Responsive → JS → DOM → React → TS → Git → API → BaaS → Projects).
...
--   * The 20260430 migration adds unique constraints on
--       modules(course_id, slug),
--       lessons(module_id, slug),
--       tasks(lesson_id, order_index)
--     so any hand-written INSERTs from the SQL Editor are protected from
--     accidental duplicates by the constraints.
--
...
--
-- PREREQUISITES (apply in this order before this seed):
--     backend/migrations/20260425_init_schema.sql
--     backend/migrations/20260426_admin_monitoring.sql
--     backend/migrations/20260426_backfill_missing_columns.sql
--     backend/migrations/20260427_admin_panel.sql
--     backend/migrations/20260428_pin_super_admin.sql
--     backend/migrations/20260429_fix_admin_list_users_ambiguous.sql
--     backend/migrations/20260430_progress_avatar_security.sql
-- =============================================================================
...
-- -----------------------------------------------------------------------------
INSERT INTO public.courses (title, slug, description, level, is_published, order_index) VALUES
  ('HTML Basics',                'html-basics',              'Семантическая разметка, теги, формы, доступность. Фундамент любого фронтенда.',                  'beginner',      true,  1),
  ('CSS Basics',                 'css-basics',               'Селекторы, box-model, flex, цвета и шрифты. Учимся стилизовать страницы.',                       'beginner',      true,  2),
  ('Responsive Layout',          'responsive-layout',        'Адаптивная вёрстка: media queries, mobile-first, CSS Grid и Flexbox для разных экранов.',        'intermediate',  true,  3),
  ('JavaScript Basics',          'javascript-basics',        'Синтаксис JS: переменные, типы, функции, массивы, объекты, async/await.',                        'beginner',      true,  4),
  ('DOM & Events',               'dom-events',               'Работа с DOM, события, формы, делегирование. Делаем страницы интерактивными.',                   'intermediate',  true,  5),
  ('React Basics',               'react-basics',             'Компоненты, JSX, props, useState, useEffect, списки, формы в React 18.',                         'intermediate',  true,  6),
  ('TypeScript Basics',          'typescript-basics',        'Типы, интерфейсы, дженерики, типизация props/state в React.',                                    'intermediate',  true,  7),
  ('Git & GitHub',               'git-github',               'Системы контроля версий: коммиты, ветки, merge, conflict, pull request.',                        'beginner',      true,  8),
  ('API Basics',                 'api-basics',               'HTTP, REST, fetch, async/await, обработка ошибок, JSON, заголовки.',                             'intermediate',  true,  9),
  ('Supabase / Firebase Basics', 'supabase-firebase-basics', 'BaaS: Auth, Database, Storage. Подключение Supabase и Firebase к фронту.',                       'intermediate',  true, 10),
  ('Frontend Projects',          'frontend-projects',        'Финальные проекты: лендинг, SPA-дашборд, pet-проект с интеграцией всего стека.',                  'advanced',      true, 11)
